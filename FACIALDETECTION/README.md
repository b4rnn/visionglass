...
Detected faces using given threshold from video frames from CCTV cameras and publish them to various nodes.

# SETUP

*FACIALDETECTION has been optimised to run on cpu . No need for GPU.*
# Dependencies
```
...
cd to  FACIALDETECTION FOLDER 
    ...
    cd build  && rm -rf CMakeCache.txt ENGINE  CMakeFiles LIVECAM Makefile  __pycache__  cmake_install.cmake 
    cmake ..
    make -j{NUMBER_OF_THREADS}
#If successful ... you will have the compiled project name i.e LIVECAM
    CMakeCache.txt  CMakeFiles  LIVECAM app.ini  Makefile  app.py  cmake_install.cmake  libfacedetect.so wsgi.py
#start flask server
    uwsgi --ini app.ini
```
