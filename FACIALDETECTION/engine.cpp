...
int main(int argc, char* argv[])
{
    ...
    fprintf(stderr, "About to spawn facial detection process from pid %d\n", getpid());
    ...
    int r = posix_spawn(&p, "./FACIALDETECTION", nullptr, nullptr,
                       (char**) args, nullptr);

    fprintf(stderr, "Child pid %d should run my facial detection\n", p);
    char * FACIALDETECTIONPID =(char*) malloc(6);
    sprintf(FACIALDETECTIONPID, "%d", p);
    ...
    if(maybe_result) {
        ...
        db["live_cams"].update_one(document{} << "_id" << bsoncxx::oid{capture_destination[0]}<< finalize,
            document{}<<"$set" << bsoncxx::builder::stream::open_document <<"facial_detection_pid"
            << FACIALDETECTIONPID << bsoncxx::builder::stream::close_document
            <<finalize
        );
    }

    std::cout << "MY CHILD PID " << FACIALDETECTIONPID << " ID " << capture_destination[0] <<std::endl;
