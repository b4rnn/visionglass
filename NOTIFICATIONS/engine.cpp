...
int main(int argc, char* argv[])
{
    ...
    fprintf(stderr, "About to spawn notification process from pid %d\n", getpid());
    ...
    int r = posix_spawn(&p, "./NOTIFICATIONS", nullptr, nullptr,
                       (char**) args, nullptr);

    std::cout << r << std::endl;

    fprintf(stderr, "Child pid %d should run my notification\n", p);

    char * NOTIFICATIONSPID =(char*) malloc(6);
    sprintf(NOTIFICATIONSPID, "%d", p);
    ...
    if(maybe_result) {
        ...
        db["live_cams"].update_one(document{} << "_id" << bsoncxx::oid{capture_destination[0]}<< finalize,
            document{}<<"$set" << bsoncxx::builder::stream::open_document <<"notification_pid"
            << NOTIFICATIONSPID << bsoncxx::builder::stream::close_document
            <<finalize
        );
    }else{
        
        db["pids"].insert_one(document{} 
            << "camera_id"      << capture_destination[0]
            << "notification_pid"      << NOTIFICATIONSPID
            << finalize
        );

    }

std::cout << "MY CHILD PID " << NOTIFICATIONSPID << " ID " << capture_destination[0] <<std::endl;
