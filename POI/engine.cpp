...
int main(int argc, char* argv[])
{
    ...
    fprintf(stderr, "About to spawn poi process from pid %d\n", getpid());
    ...
    int r = posix_spawn(&p, "./POI", nullptr, nullptr,
                       (char**) args, nullptr);

    std::cout << r << std::endl;

    fprintf(stderr, "Child pid %d should run my poi\n", p);

    char * POIPID =(char*) malloc(6);
    sprintf(POIPID, "%d", p);
    ...
    if(maybe_result) {
        ...
        db["live_cams"].update_one(document{} << "_id" << bsoncxx::oid{capture_destination[0]}<< finalize,
            document{}<<"$set" << bsoncxx::builder::stream::open_document <<"person_of_interest_pid"
            << POIPID << bsoncxx::builder::stream::close_document
            <<finalize
        );
    }

    std::cout << "MY CHILD PID " << POIPID << " ID " << capture_destination[0] <<std::endl;
return 0;
}
