...
mongocxx::uri uri("mongodb://DATABASE_ID:27017");
...
int main(int argc, char* argv[])
{
    ...
    char * MEDIAPLAYERPID =(char*) malloc(6);
    sprintf(MEDIAPLAYERPID, "%d", p);
    ...
    if(maybe_result) {
        ...
        db["live_cams"].update_one(document{} << "_id" << bsoncxx::oid{capture_destination[0]} << finalize,
            document{}<<"$set" << bsoncxx::builder::stream::open_document <<"media_player_pid"
            << MEDIAPLAYERPID << bsoncxx::builder::stream::close_document
            <<finalize
        );
    }

    std::cout << capture_destination[0] << " " << p << " " << MEDIAPLAYERPID << std::endl;
