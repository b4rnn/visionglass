...
Encode/Decode raw video frames as packets and publish them for display and recording as m3u8

# SETUP

*MEDIAPLAYER has been optimised to run on cpu . No need for GPU.*
# Dependencies
```
...
#Nginx Directories
    #File Directories
     cd /var/www/html/ && mkdir live && mkdir uploads && mkdir images && mkdir quotas && mkdir cbi && mkdir recordings && mkdir recordings/videos && mkdir recordings/cams && cd ..
```
# python 3.5 +
```
#update & upgrade
...
#paste text[from BROADCAST/default]
...
#If successful ... you will have the compiled project name i.e MEDIAPLAYER
    CMakeCache.txt  CMakeFiles  MEDIAPLAYER app.ini  Makefile  app.py  cmake_install.cmake  libfacedetect.so wsgi.py
#start flask server
    uwsgi --ini app.ini
```
