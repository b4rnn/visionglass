...
int main(int argc, char* argv[])
{
    ...
    fprintf(stderr, "About to spawn facial recognition process from pid %d\n", getpid());
    ...
    int r = posix_spawn(&p, "./FACIALRECOGNTION", nullptr, nullptr,
                       (char**) args, nullptr);

    std::cout << r << std::endl;

    fprintf(stderr, "Child pid %d should run my facial recognition\n", p);

    char * FACIALRECOGNTIONPID =(char*) malloc(6);
    sprintf(FACIALRECOGNTIONPID, "%d", p);
    ...
    if(maybe_result) {
        ...
        db["live_cams"].update_one(document{} << "_id" << bsoncxx::oid{capture_destination[0]} << finalize,
            document{}<<"$set" << bsoncxx::builder::stream::open_document <<"facial_recognition_pid"
            << FACIALRECOGNTIONPID << bsoncxx::builder::stream::close_document
            <<finalize
        );
    }

     std::cout << "MY CHILD PID " << FACIALRECOGNTIONPID << " ID " << capture_destination[0] <<std::endl;
