...
Extract Detected faces using given threshold from video frames from CCTV cameras and publish them to various nodes 

# SETUP

*FACIALRECOGNITION has been optimised to run on cpu . No need for GPU.*
# Dependencies
```
...
```
REPLACE [STREAM_IP] WITH ACTUAL REDIS SERVER IP I.E 192.168.8.208 [LOCATE STREAM_IP FROM main.cpp ]
...
cd to  FACIALRECOGNITION FOLDER 
    cd build  && rm -rf CMakeCache.txt ENGINE  CMakeFiles FACIALRECOGNITION Makefile  __pycache__  cmake_install.cmake 
    cmake ..
    make -j{NUMBER_OF_THREADS}
#If successful ... you will have the compiled project name i.e FACIALRECOGNITION
    CMakeCache.txt  CMakeFiles  FACIALRECOGNITION app.ini  Makefile  app.py  cmake_install.cmake  libfacedetect.so wsgi.py
#start flask server
    uwsgi --ini app.ini
```
