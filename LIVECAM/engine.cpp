...
int main(int argc, char* argv[])
{
    ...
    fprintf(stderr, "About to spawn livecam process from pid %d\n", getpid());
    ...
    int r = posix_spawn(&p, "./LIVECAM", nullptr, nullptr,
                       (char**) args, nullptr);

    fprintf(stderr, "Child pid %d should run my livecam\n", p);

    char * LIVECAMPID =(char*) malloc(6);
    sprintf(LIVECAMPID, "%d", p);
    ...
    if(maybe_result) {
        ...
        db["live_cams"].update_one(document{} << "_id" << bsoncxx::oid{capture_destination[0]} << finalize,
            document{}<<"$set" << bsoncxx::builder::stream::open_document <<"live_cam_pid"
            << LIVECAMPID << bsoncxx::builder::stream::close_document
            <<finalize
        );
    }

    //switch live came status from OFF to ON
    bsoncxx::stdx::optional<bsoncxx::document::value> _result = db["live_cams"].find_one(document{} << "_id" << bsoncxx::oid{capture_destination[0]} << finalize);
    if(_result) {
        db["live_cams"].update_one(document{} << "_id" << bsoncxx::oid{capture_destination[0]} << finalize,
            document{}<<"$set" << bsoncxx::builder::stream::open_document <<"cam_status"
            << "ON" << bsoncxx::builder::stream::close_document
            <<finalize
        );
    }

    std::cout << capture_destination[0] << " " << p << " " << LIVECAMPID << " ID " << capture_destination[0] << std::endl;
